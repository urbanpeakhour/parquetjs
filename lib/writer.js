  ...
  _transform(row, encoding, callback) {
    if (row) {
      this.writer.appendRow(row).then(
        ...
        err => {
          const fullErr = new Error(`Error transforming to parquet: ${err.toString()} row:${JSON.stringify(row, (key, value) =>
            // Handle bigint values. See https://github.com/GoogleChromeLabs/jsbi/issues/30#issuecomment-521460510
            typeof value === 'bigint'
                ? value.toString()
                : value)}`);
          fullErr.origErr = err;
